...
// Anf if we've played, turn the LED on
        else if (msg.state.zoneState == "PLAYING") powermate.setBrightness(PLAY_BRIGHTNESS);
    }
});
...
// Figure out if our player is playing. If so, turn the LED on
    if (isPlaying()) {
        powermate.setBrightness(PLAY_BRIGHTNESS);
...
function getFaveAudio(index) {
    ...
    var link = voiceRssLink(VOICERSS_API_KEY, faves[index]);
